...
namespace Encog.ML.Data.Market
{
    ...
    /// </summary>
    class MLFinanceDataSet :TemporalMLDataSet
    {
        public MLFinanceDataSet(int inputWindowSize, int predictWindowSize) : base(inputWindowSize, predictWindowSize)
        {
        }
        ...
        public MLFinanceDataSet(IMarketLoader loader,
        ...
        public MLFinanceDataSet(IMarketLoader loader, int inputWindowSize, int predictWindowSize, TimeUnit unit)
            : base(inputWindowSize, predictWindowSize)
        {
        ...
        {
            if (!(desc is FinanceDataDescription))
            {
        ...
        public void Load(DateTime begin, DateTime end)
        {
            ...
            IDictionary<FinanceSymbols.Instrument, object> set = new Dictionary<FinanceSymbols.Instrument, object>();
            foreach (TemporalDataDescription desc in Descriptions)
            {
                var mdesc = (FinanceDataDescription) desc;
                set[mdesc.Ticker] = null;
            }

            // now loop over each symbol and load the data
            foreach (FinanceSymbols.Instrument symbol in set.Keys)
            {
        ...
        private void LoadSymbol(FinanceSymbols.Instrument ticker, DateTime from,
                                DateTime to)
        {
